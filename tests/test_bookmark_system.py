...
class BookmarkSystemTest(unittest.TestCase):
    ...
    def test_rebuild_visualize_and_validate(self):
        ...
        self.assertIn('id="mode-toggle"', visualizer)
        self.assertIn('role="switch"', visualizer)
        ...
        self.assertIn('Find Your Stuff', visualizer)
        self.assertNotIn('Ways in', visualizer)
        self.assertIn('id="inspector-backdrop"', visualizer)
        self.assertIn("setInspectorOpen(false,true)", visualizer)
        self.assertIn("event.key === 'Escape'", visualizer)
        ...
        self.assertIn('class="maker-logo-light"', visualizer)
        self.assertEqual(visualizer.count('data:image/png;base64,'), 3)
        self.assertTrue((REPO_ROOT / "skills" / "bookmark-this" / "assets" / "bookmark-this-mark.svg").is_file())
        ...
        self.assertIn(".workspace { min-width: 0; background-color: var(--paper); background-image:", template)
        self.assertIn("background-position: 52% center;", template)
        self.assertIn("padding: var(--space-lg) var(--space-2xl) var(--space-3xl); background: transparent;", template)
        self.assertIn("const modes = new Set(['light','dark']);", template)
        self.assertIn("let mode = 'dark';", template)
        self.assertIn(':root[data-mode="light"]', template)
        self.assertIn('applyMode(state.mode === \'dark\' ? \'light\' : \'dark\')', template)
