---
name: bookmark-this
description: Set up and operate a user-owned Markdown bookmark system that verifies URLs, prevents duplicates, writes structured bookmark notes, and generates a central index with category and tag navigation. Use when a user says "set up my bookmark system," "bookmark this," "save this link," asks to organize saved URLs, or wants to rebuild or validate their bookmark hierarchy.
---
...
Never assume a personal path, name, taxonomy, note-taking app, or project context.
...
1. Summarize the preferred agentic platform, source folders, proposed root, hierarchy, categories, visualizer choice, capture depth, inference policy, and migration scope.
...
6. Run `python3 scripts/bookmark_system.py rebuild <root>`. If the visualizer is enabled, run `python3 scripts/bookmark_system.py visualize <root>`.
7. Run `python3 scripts/bookmark_system.py validate <root>`.
8. Report the created hierarchy, configuration choices, validation result, visualizer path when enabled, and three platform-appropriate example prompts.
